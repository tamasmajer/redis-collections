...
class MockIdToSet extends RedisIdToSet {
    ...
    findKeys() {
        ...
        const ix = template.indexOf('*')
        const prefix = template.substring(0, ix)
        const postfix = template.substring(ix + 1)
        const keys = Object.keys(data || {})
        return keys.filter(k => k.startsWith(prefix) && k.endsWith(postfix))
    }
